...
* **X11/Compiz Transparency Fix:** Resolved a severe hardware-acceleration issue where the UI would render completely transparently or only become opaque upon mouse hover on certain desktop environments. Added an explicit `GSK_RENDERER=software` fallback engine pipeline to force clean rendering on non-GNOME X11 compositors.
